...
Info:
- Modefit.BaseFitter is minimizing `-2*self.model.get_logprob(*self._get_model_args_)`
"""
...
class RateFitter( BaseFitter ):
    ...
    def set_fitted_flag(self, fitted_flag):
        ...
        if len(fitted_flag) != self.ndata:
            raise ValueError("Size of the data (%d) do not match that of the given fitted_flag (%d)"%(self.ndata,len(fitted_flag)))
        self._side_properties["fitted_flag"] = fitted_flag
        self._derived_properties["fitted_counts"] = self.counts[self.fitted_flag]
        self._derived_properties["fitted_redshift_ranges"] = self.redshift_ranges.T[self.fitted_flag].T
        
    # --------- #
    #  PLOTTER  #
    # --------- #
    def show(self, datacolor="C0", modelcolor="C1", stepalpha=0.2, add_proba=True):
        """ """
        import matplotlib.pyplot as plt
        fig = plt.figure(figsize=[5,3])
        ax  = fig.add_axes([0.12,0.15,1-0.12*2,0.75])
        ax.step(self._central_redshiftranges, self.counts, where="mid", alpha=stepalpha)
        
        if self.fitted_flag is not None and not np.all(self.fitted_flag):
            prop = dict(marker="o", facecolors="None", edgecolors=datacolor)
            
            ax.scatter(self._central_redshiftranges[~self.fitted_flag], self.counts[~self.fitted_flag], **prop)
            prop["edgecolors"] = modelcolor
            ax.scatter(self._central_redshiftranges[~self.fitted_flag], self.get_model(self.redshift_ranges.T[~self.fitted_flag].T),**prop)
            
        prop = dict(marker="o")
        ax.scatter(self._central_redshiftranges[self.fitted_flag], self.fitted_counts, c=datacolor, **prop)
        ax.scatter(self._central_redshiftranges[self.fitted_flag], self.get_model(self.fitted_redshift_ranges),c=modelcolor, **prop)
        ...
        if add_proba:
            ...
            if self.fitted_flag is not None and not np.all(self.fitted_flag):
                axt.plot(self._central_redshiftranges, 
                        self.model.get_probabilities(self.counts, self.redshift_ranges),
                        color="0.5",ls="--")
                
                axt.plot(self._central_redshiftranges[self.fitted_flag], 
                    self.model.get_probabilities(self.fitted_counts, self.fitted_redshift_ranges),
                             color="0.5",ls="-")
            else:
                axt.plot(self._central_redshiftranges, 
                        self.model.get_probabilities(self.counts, self.redshift_ranges),
                        color="0.5",ls="-")
            axt.set_ylabel("Poisson pmf")
    ...
    def fitted_redshift_ranges(self):
        ...
        if self._derived_properties["fitted_redshift_ranges"] is None:
            self._derived_properties["fitted_redshift_ranges"] = self.redshift_ranges
...
class _RateModelStructure_( BaseModel ):
    ...
    def setup(self, parameters):
        ...
        if self.nmissedparameters>0:
            self.paramrate   = {k:v for k,v in zip(self.RATEPARAMETERS, parameters[:self.nrateparameters])}
            self.parammissed = {k:v for k,v in zip(self.MISSEDPARAMETERS, parameters[self.nrateparameters:])}
        elif self.nrateparameters==1:
            self.paramrate   = {self.RATEPARAMETERS[0]: parameters}
        else:
            self.paramrate   = {k:v for k,v in zip(self.RATEPARAMETERS, parameters)}
    ...
    def get_expectedrate(self, redshift_ranges):
        """ """
        return self.get_rate( redshift_ranges ) - self.get_missedrate( redshift_ranges )

    def get_rate(self, redshift_ranges):
        """ """
        # Could use self.paramrate
        raise NotImplementedError("Your RateModel class must define `get_rate()` ")

    def get_missedrate(self, redshift_ranges):
        """ """
        # Could use self.missedrate
        raise NotImplementedError("Your RateModel class must define `get_missedrate()` ")
    ...
    def get_loglikelihood(self, counts, redshift_ranges ):
        """ """
        # poisson.pmf(k, mu) = probability to observe k counts given that we expect mu
...
class VolumeRateModel( BaseObject ):
    ...
    def _get_rate_(self, redshifts):
        """ """
        return self.paramrate['a']* cosmo.comoving_volume(redshifts).value/self.VOLUME_SCALE
    
    def get_rate(self, redshift_ranges):
        """ """
        # Could use self.paramrate
        return self._get_rate_(redshift_ranges[1]) -self._get_rate_(redshift_ranges[0])
